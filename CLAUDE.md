...
- **Sections:** I. **Discursos** (the home feed → free on the Substack newsletter; currently EMPTY on purpose — see below) · II. **Libros** (Quema Tu Dinero on Amazon; more coming) · **Historias** (added 2026-09-14 — his public sci-fi/fiction, one YouTube video embedded per entry; free, not gated) · III. **Escritos Secretos** (writings about **consciousness and truth** — "escritos sobre la conciencia y la verdad"; reserved, each text needs its own aporte, apply via navirobayo@gmail.com — **no generic payment button here, Ivan removed it twice on purpose, don't re-add it**) · **Revelaciones de la Verdad** (the aporte page, now just "La Verdad. Directa." as of 2026-09-14) · **Acerca de**. ⚠️ The podcast was removed (2026-07-21) — do not re-add it anywhere. Category-page close line (Ivan's): "Ivan Robayo es un escritor y consejero personal de CEOs, Atletas Profesionales, Celebridades al rededor del mundo."
- **Historias (historias.html):** public fiction, one embedded YouTube video per `<article class="post">` using `.post-embed` (16:9 iframe, replaces `.post-img`). Publish the same way as the home feed: copy the PLANTILLA comment block in the file, paste as a new entry, swap in the video ID/title/subtitle. Shipped empty (no fabricated placeholder entries) — waiting on Ivan to supply the actual video list.
...
├── libros.html         — Category page: books (Quema Tu Dinero → Amazon). Ivan's copy: "Para el buscador de Verdad".
├── historias.html      — Category page: his fiction, embedded YouTube videos. Publish via the PLANTILLA comment block.
